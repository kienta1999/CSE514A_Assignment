...
model.fit(X_train, y_train_onehot, epochs = 50, callbacks=[callback])

val_loss, val_acc = model.evaluate(X_val, y_val_onehot, verbose=2)
...
print("Model accuracy on val: {:5.2f}%".format(100 * val_acc))
print("Model loss on val", val_loss)
# print(np.rint(model.predict(X_val)))
